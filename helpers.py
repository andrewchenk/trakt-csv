...
def read_config(_trakt, options):
    ...
    if os.path.exists(_configfile):
        try:
            ...
            if config.has_option('TRAKT', 'CLIENT_ID') and len(
                    config.get('TRAKT', 'CLIENT_ID')) != 0:
                _trakt['client_id'] = config.get('TRAKT', 'CLIENT_ID')
            else:
                print('Error, you must specify a trakt.tv CLIENT_ID')
                sys.exit(1)
            if config.has_option('TRAKT', 'CLIENT_SECRET') and len(
                    config.get('TRAKT', 'CLIENT_SECRET')) != 0:
            ...
            if config.has_option(
                    'TRAKT',
                    'USERNAME') and len(config.get('TRAKT', 'USERNAME')) != 0:
                _trakt['username'] = config.get('TRAKT', 'username')
            else:
                print(
                    'Error, you must specify a trakt.tv username for user lists to work'
                )
                sys.exit(1)
            if config.has_option('TRAKT', 'ACCESS_TOKEN') and len(
                    config.get('TRAKT', 'ACCESS_TOKEN')) != 0:
                _trakt['access_token'] = config.get('TRAKT', 'ACCESS_TOKEN')
            else:
                print(
                    'Warning, no access token found. Authentification is required'
                )
            if config.has_option('TRAKT', 'REFRESH_TOKEN') and len(
                    config.get('TRAKT', 'REFRESH_TOKEN')) != 0:
            ...
            if _proxy['proxy'] and config.has_option(
                    'SETTINGS', 'PROXY_HOST') and config.has_option(
                        'SETTINGS', 'PROXY_PORT'):
...
def api_auth_refresh(_trakt, _headers, options):
    ...
    if _trakt['refresh_token']:
        ...
        if r.status_code == 200:
            ...
            print(
                "Refreshing access_token failed. Get new refresh_token and access_token by manually authenticating again"
            )
            api_auth(_trakt, _headers, options)
    else:
        print(
            "No refresh_token found in config file. Get new refresh_token and access_token by manually authenticating again"
        )
        api_auth(_trakt, _headers, options)
